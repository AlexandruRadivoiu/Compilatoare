...
public class CChecker {
                ...
                for (int i = 0; i < size; i++) { //pentru fiecare valoare din key
                    ...
                    for (int t = 1; t <= x.length(); t++) {
                     ...
                     /*   if (nrLitereMatchNou < nrLitereMatch) {
                        ...
                        if (nrLitereMatch != 0 && t == x.length()) {
                            Map<String, List<String>> returnedMap = CChecker.doIt(nrLitereMatch, listaProblemeActuale, entry.getKey());
                            for (Map.Entry<String, List<String>> tempMap : returnedMap.entrySet()) {
                                if (!newMap.containsKey(tempMap.getKey())) {
                                    newMap.put(tempMap.getKey(), tempMap.getValue());
                                } else
    ...
    public static Map<String, List<String>> doMap(List<Reguli> reguliProductie) {
        Map<String, List<String>> returnMap = new HashMap<>();
        List<String> listaNeterminale = new ArrayList<>();
        for (Reguli regula : reguliProductie) {
            List<String> list = new ArrayList<>();
            if (!listaNeterminale.contains(regula.stanga)) {
                listaNeterminale.add(regula.stanga);
                list.add(regula.dreapta);
                returnMap.put(regula.stanga, list);
            } else
                returnMap.get(regula.stanga).add(regula.dreapta);
        }
        return returnMap;
    }
